...
void test_basic_arithmetic(TestContext& context) {
    ...
    const auto double_subtract = rvemu::core::floating_subtract(Format::Double,
                                                                0x4018'0000'0000'0000ULL,
                                                                0x4000'0000'0000'0000ULL,
                                                                Mode::NearestTiesToEven);
    context.expect(double_subtract.bits == 0x4010'0000'0000'0000ULL, "binary64 6-2 必须精确得到 4");
    ...
    const auto quotient_rtz =
        rvemu::core::floating_divide(Format::Single, 0x3F80'0000U, 0x4040'0000U, Mode::TowardZero);
    context.expect(quotient_rne.bits == 0x3EAA'AAABU, "binary32 1/3 的 RNE 末位必须向偶数方向进位");
    context.expect(quotient_rtz.bits == 0x3EAA'AAAAU, "binary32 1/3 的 RTZ 必须截去无限尾部");
    ...
    context.expect(
        irrational_root.bits == 0x3FF6'A09E'667F'3BCDULL
            && irrational_root.flags == flag(rvemu::core::FloatingExceptionFlag::Inexact),
        "binary64 sqrt(2) 必须得到规范 RNE 位模式并设置 NX");

    const auto double_third = rvemu::core::floating_divide(Format::Double,
...
void test_fused_and_rounding(TestContext& context) {
    ...
    const auto fused = rvemu::core::floating_fused_multiply_add(Format::Single,
                                                                0x3F80'0001U,
                                                                0x3F80'0001U,
                                                                0xBF80'0002U,
                                                                false,
    ...
    const auto fma_tiny_after_rounding = rvemu::core::floating_fused_multiply_add(
        Format::Single, 0x0080'0000U, 0x32C1'1A50U, 0x007F'FFFFU, false, false, Mode::Up);
    const auto underflow_flags =
        static_cast<std::uint8_t>(flag(rvemu::core::FloatingExceptionFlag::Underflow)
    ...
    const auto fma_not_tiny_after_rounding = rvemu::core::floating_fused_multiply_add(
        Format::Single, 0x3F00'FBFFU, 0x8000'0001U, 0x807F'FFFFU, false, false, Mode::Down);
    ...
    const auto rmm = rvemu::core::floating_from_integer(
        Format::Single, 0x0100'0001U, true, 32U, Mode::NearestTiesToMaximumMagnitude);
    ...
    const auto negative_down =
        rvemu::core::floating_add(Format::Single, 0xBF80'0000U, 0xB300'0000U, Mode::Down);
    const auto negative_up =
        rvemu::core::floating_add(Format::Single, 0xBF80'0000U, 0xB300'0000U, Mode::Up);
    context.expect(negative_down.bits == 0xBF80'0001U, "RDN 对负数非精确和必须向负无穷增加幅值");
    context.expect(negative_up.bits == 0xBF80'0000U, "RUP 对负数非精确和必须向正无穷取较小幅值");
}
...
void test_special_values_and_flags(TestContext& context) {
    ...
    const auto divide_zero =
        rvemu::core::floating_divide(Format::Single, 0xBF80'0000U, 0U, Mode::NearestTiesToEven);
    context.expect(divide_zero.bits == 0xFF80'0000U, "负有限数除正零必须得到负无穷");
    ...
    const auto overflow_flags =
        static_cast<std::uint8_t>(flag(rvemu::core::FloatingExceptionFlag::Overflow)
                                  | flag(rvemu::core::FloatingExceptionFlag::Inexact));
    ...
    const auto negative_root =
        rvemu::core::floating_square_root(Format::Single, 0xBF80'0000U, Mode::NearestTiesToEven);
    context.expect(
        negative_root.bits == rvemu::core::kCanonicalSingleNan
...
void test_non_arithmetic_operations(TestContext& context) {
    const auto injected =
        rvemu::core::floating_sign_inject(Format::Single, 0x3F80'0000U, 0xBF00'0000U, 0U);
    context.expect(injected == 0xBF80'0000U, "FSGNJ 必须只复制符号位");

    const auto minimum =
        rvemu::core::floating_minimum_maximum(Format::Single, 0x7FC0'1234U, 0x4000'0000U, false);
    context.expect(minimum.bits == 0x4000'0000U && minimum.flags == 0U,
                   "FMIN 遇到单个 qNaN 必须返回数值操作数");
    const auto signaling_minimum =
        rvemu::core::floating_minimum_maximum(Format::Single, 0x7F80'0001U, 0x4000'0000U, false);
    context.expect(signaling_minimum.bits == 0x4000'0000U
                       && signaling_minimum.flags
                              == flag(rvemu::core::FloatingExceptionFlag::InvalidOperation),
                   "FMIN 遇到 sNaN 必须返回数值操作数并设置 NV");

    const auto quiet_compare =
        rvemu::core::floating_compare(Format::Double,
                                      rvemu::core::kCanonicalDoubleNan,
                                      0x3FF0'0000'0000'0000ULL,
                                      rvemu::core::FloatingComparison::Equal);
    const auto signaling_compare =
        rvemu::core::floating_compare(Format::Double,
                                      rvemu::core::kCanonicalDoubleNan,
                                      0x3FF0'0000'0000'0000ULL,
...
void test_conversions(TestContext& context) {
    const auto narrowed = rvemu::core::floating_convert_format(
        Format::Single, Format::Double, 0x3FF8'0000'0000'0000ULL, Mode::NearestTiesToEven);
    const auto widened = rvemu::core::floating_convert_format(
        Format::Double, Format::Single, 0xBFC0'0000U, Mode::NearestTiesToEven);
    ...
    const auto nan_to_unsigned = rvemu::core::floating_to_integer(
        Format::Double, rvemu::core::kCanonicalDoubleNan, true, 64U, Mode::NearestTiesToEven);
    context.expect(
        nan_to_unsigned.value == 0xFFFF'FFFF'FFFF'FFFFULL
