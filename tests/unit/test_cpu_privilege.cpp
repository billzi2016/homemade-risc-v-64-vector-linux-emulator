...
[[nodiscard]] constexpr std::uint32_t encode_csr(std::uint32_t function3,
                                                 std::uint32_t destination,
                                                 std::uint32_t source_or_immediate,
                                                 rvemu::core::CsrAddress address) noexcept {
    return (static_cast<std::uint32_t>(address) << 20U) | ((source_or_immediate & 0x1FU) << 15U)
           | ((function3 & 0x7U) << 12U) | ((destination & 0x1FU) << 7U) | 0x73U;
}

// 组装一条基础 ADDI，用来验证 WFI 唤醒后的真实后继指令执行。
[[nodiscard]] constexpr std::uint32_t encode_addi(std::uint32_t destination,
                                                  std::uint32_t source,
                                                  std::uint32_t immediate) noexcept {
    return ((immediate & 0xFFFU) << 20U) | ((source & 0x1FU) << 15U) | ((destination & 0x1FU) << 7U)
           | 0x13U;
}
   ...
   public:
    ...
    CpuFixture()
        : ram_(std::make_shared<rvemu::memory::PhysicalMemory>(
              rvemu::bus::PhysicalAddress{kRamBase}, 0x2000U, "privilege-test-ram")),
          cpu_(bus_) {
    ...
    void write_instruction(std::uint32_t bits, std::uint64_t address) {
        const auto written = bus_.write(rvemu::bus::PhysicalAddress{address},
...
void test_reset_permissions_and_warl(TestContext& context) {
    ...
    context.expect(csrs.peek(rvemu::core::CsrAddress::Misa) == misa,
                   "misa 必须只读回真实实现扩展集合");

    write_csr(csrs, rvemu::core::CsrAddress::Mstatus, 0x2ULL << 11U);
    context.expect(((csrs.peek(rvemu::core::CsrAddress::Mstatus) >> 11U) & 0x3U) == 0x3U,
                   "MPP 保留编码必须归一为已实现特权级");
    ...
    context.expect(csrs.peek(rvemu::core::CsrAddress::Satp) == sv39,
                   "satp 不支持 MODE 写入必须整体无效");

    write_csr(csrs, rvemu::core::CsrAddress::Medeleg, ~0ULL);
    const auto expected_exceptions = bit(0U) | bit(1U) | bit(2U) | bit(3U) | bit(4U) | bit(5U)
                                     | bit(6U) | bit(7U) | bit(8U) | bit(9U) | bit(12U) | bit(13U)
                                     | bit(15U);
    context.expect(csrs.peek(rvemu::core::CsrAddress::Medeleg) == expected_exceptions,
                   "medeleg 只允许真实可委托异常位");
    write_csr(csrs, rvemu::core::CsrAddress::Mideleg, ~0ULL);
    context.expect(csrs.peek(rvemu::core::CsrAddress::Mideleg) == (bit(1U) | bit(5U) | bit(9U)),
                   "mideleg 只允许监督级中断位");
}
...
void test_real_csr_instructions(TestContext& context) {
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Mscratch) == 0xAAU,
                   "CSRRW 必须写入新值");
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Mscratch) == 0xFFU,
                   "CSRRS 必须设置掩码位");
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Mscratch) == 0xF0U,
                   "CSRRC 必须清除掩码位");

    result = fixture.execute(encode_csr(5U, 0U, 3U, rvemu::core::CsrAddress::Mscratch));
    expect_retired(context, result, "CSRRWI rd=x0");
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Mscratch) == 3U,
                   "CSRRWI 必须使用五位零扩展立即数");
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Mscratch) == 6U,
                   "CSRRCI 必须清除指定立即数位");
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Misa) == misa_before,
                   "misa 写入不得宣称未实现扩展");
...
void test_supervisor_aliases(TestContext& context) {
    ...
    context.expect(
        read_csr(csrs, rvemu::core::CsrAddress::Sie, rvemu::core::PrivilegeMode::Supervisor)
            == supervisor_mask,
        "sie 必须是 mie 与 mideleg 的受限视图");

    write_csr(csrs, rvemu::core::CsrAddress::Sie, bit(1U), rvemu::core::PrivilegeMode::Supervisor);
    context.expect(csrs.peek(rvemu::core::CsrAddress::Mie) == (machine_mask | bit(1U)),
                   "写 sie 必须只修改已委托 mie 位并保留机器位");
    ...
    context.expect(
        read_csr(csrs, rvemu::core::CsrAddress::Sip, rvemu::core::PrivilegeMode::Supervisor)
            == supervisor_mask,
        "sip 必须投影已委托 mip 状态");
    write_csr(csrs, rvemu::core::CsrAddress::Sip, 0U, rvemu::core::PrivilegeMode::Supervisor);
    context.expect((csrs.peek(rvemu::core::CsrAddress::Mip) & bit(1U)) == 0U,
                   "sip 只允许软件清除 SSIP");
    context.expect(
        (csrs.peek(rvemu::core::CsrAddress::Mip) & (bit(5U) | bit(9U))) == (bit(5U) | bit(9U)),
        "写 sip 不得清除设备 pending 位");

    write_csr(csrs, rvemu::core::CsrAddress::Mstatus, bit(3U) | (0x3ULL << 11U));
    write_csr(csrs,
              rvemu::core::CsrAddress::Sstatus,
              bit(1U) | bit(18U),
              rvemu::core::PrivilegeMode::Supervisor);
    const auto mstatus = csrs.peek(rvemu::core::CsrAddress::Mstatus);
    context.expect((mstatus & bit(1U)) != 0U && (mstatus & bit(18U)) != 0U,
                   "写 sstatus 必须更新共享 S 字段");
    context.expect((mstatus & bit(3U)) != 0U && ((mstatus >> 11U) & 0x3U) == 3U,
                   "写 sstatus 不得破坏 M-only 字段");
}
...
void test_counters_and_access_control(TestContext& context) {
    ...
    context.expect(read_csr(csrs, rvemu::core::CsrAddress::Time) == 0x1234'5678U,
                   "M-mode 必须读取 CLINT 投影的 time");
    ...
    context.expect(
        read_csr(csrs, rvemu::core::CsrAddress::Cycle, rvemu::core::PrivilegeMode::Supervisor)
            == 0U,
        "mcounteren 放行后 S-mode 必须读取 cycle");
    ...
    write_csr(
        csrs, rvemu::core::CsrAddress::Scounteren, bit(2U), rvemu::core::PrivilegeMode::Supervisor);
    context.expect(
        read_csr(csrs, rvemu::core::CsrAddress::Instret, rvemu::core::PrivilegeMode::User) == 0U,
        "两级 counteren 同时放行后 U-mode 必须读取 instret");

    CpuFixture fixture;
    auto& state = fixture.cpu().state();
    auto result = fixture.execute(encode_addi(1U, 0U, 1U));
    expect_retired(context, result, "计数器普通退休指令");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mcycle) == 1U,
                   "普通步进必须增加 mcycle");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Minstret) == 1U,
                   "退休指令必须增加 minstret");

    result = fixture.execute(0xFFFF'FFFFU);
    expect_illegal(context, result, "计数器非法指令");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mcycle) == 2U,
                   "异常指令仍消耗一个模拟 cycle");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Minstret) == 1U,
                   "异常指令不得增加 minstret");
    ...
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Minstret) == 100U,
                   "显式 minstret 写必须抑制同指令隐式递增");
...
void test_machine_trap_and_mret(TestContext& context) {
    ...
    context.expect(delivery.target == rvemu::core::PrivilegeMode::Machine,
                   "未委托异常必须进入 M-mode");
    context.expect(state.privilege() == rvemu::core::PrivilegeMode::Machine,
                   "Trap 后当前模式必须为 M");
    context.expect(state.program_counter() == kTrapBase, "Direct mtvec 必须使用 BASE");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mepc) == kRamBase,
                   "mepc 必须保存故障 PC");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mcause) == 9U,
                   "mcause 必须记录 S-mode ECALL");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Scause) == 0x55U,
                   "M Trap 不得修改监督级 Trap CSR");
    const auto trapped_status = state.csrs().peek(rvemu::core::CsrAddress::Mstatus);
    context.expect((trapped_status & bit(3U)) == 0U && (trapped_status & bit(7U)) != 0U,
                   "M Trap 必须压栈 MIE 到 MPIE");
    ...
    context.expect(state.privilege() == rvemu::core::PrivilegeMode::Supervisor,
                   "MRET 必须恢复 S-mode");
    ...
    context.expect((returned_status & bit(3U)) != 0U && (returned_status & bit(7U)) != 0U,
                   "MRET 必须恢复 MIE 并置 MPIE");
...
void test_delegated_trap_and_sret(TestContext& context) {
    ...
    const auto ecall = fixture.execute(0x0000'0073U);
    const auto delivery = fixture.cpu().take_trap(*ecall.trap);
    context.expect(delivery.target == rvemu::core::PrivilegeMode::Supervisor,
                   "已委托 U ECALL 必须进入 S-mode");
    context.expect(state.program_counter() == kTrapBase, "同步异常在 Vectored 模式仍必须进入 BASE");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Sepc) == kRamBase,
                   "sepc 必须保存 U-mode 故障 PC");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Scause) == 8U,
                   "scause 必须记录 U-mode ECALL");
    context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mcause) == 0x66U,
                   "S Trap 不得修改机器级 Trap CSR");
    const auto trapped_status = state.csrs().peek(rvemu::core::CsrAddress::Mstatus);
    context.expect((trapped_status & bit(1U)) == 0U && (trapped_status & bit(5U)) != 0U,
                   "S Trap 必须压栈 SIE 到 SPIE");
    ...
    context.expect(state.privilege() == rvemu::core::PrivilegeMode::User,
                   "SRET 必须按 SPP 恢复 U-mode");
    ...
    context.expect(machine_delivery.target == rvemu::core::PrivilegeMode::Machine,
                   "M-mode Trap 不得向下委托");
}
...
void test_complete_synchronous_exception_delegation(TestContext& context) {
    ...
    for (const auto cause : implemented_causes) {
        ...
        const auto is_delegatable =
            cause != rvemu::core::ExceptionCause::EnvironmentCallFromMachine;
        for (const auto source : lower_sources) {
            ...
            const auto delivery =
                fixture.cpu().take_trap(rvemu::core::Trap{cause, fault_value, fault_pc, 0U});
            const auto expected_target = is_delegatable ? rvemu::core::PrivilegeMode::Supervisor
                                                        : rvemu::core::PrivilegeMode::Machine;
            context.expect(delivery.target == expected_target,
                           "每种同步异常必须按 medeleg 和来源特权级选择规范目标");
            context.expect(state.program_counter()
                               == (expected_target == rvemu::core::PrivilegeMode::Supervisor
                                       ? kTrapBase
                                       : kSecondTrapBase),
                           "同步异常必须跳转到目标层 Direct tvec 基址");

            if (expected_target == rvemu::core::PrivilegeMode::Supervisor) {
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Sepc) == fault_pc,
                               "委托异常必须写 sepc");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Scause) == cause_code,
                               "委托异常必须写 scause");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Stval) == fault_value,
                               "委托异常必须写 stval");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mepc) == 0x1111'0000U,
                               "S Trap 不得修改 mepc");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mcause) == 0x1111'0001U,
                               "S Trap 不得修改 mcause");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mtval) == 0x1111'0002U,
                               "S Trap 不得修改 mtval");
            } else {
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mepc) == fault_pc,
                               "未委托异常必须写 mepc");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mcause) == cause_code,
                               "未委托异常必须写 mcause");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mtval) == fault_value,
                               "未委托异常必须写 mtval");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Sepc) == 0x2222'0000U,
                               "M Trap 不得修改 sepc");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Scause) == 0x2222'0001U,
                               "M Trap 不得修改 scause");
                context.expect(state.csrs().peek(rvemu::core::CsrAddress::Stval) == 0x2222'0002U,
                               "M Trap 不得修改 stval");
            }
        }
    }
        ...
        for (const auto delegated : {false, true}) {
            ...
            for (const auto source : {rvemu::core::PrivilegeMode::Machine,
                ...
                for (const auto global_enabled : {false, true}) {
                    ...
                    write_csr(
                        csrs, rvemu::core::CsrAddress::Mstatus, global_enabled ? global_bit : 0U);
                    ...
                    const auto should_accept =
                        !blocked_by_machine_mode
                        && (target_is_higher || (target_is_current && global_enabled));

                    context.expect(selected.has_value() == should_accept,
...
void test_interrupt_trap_state_isolation(TestContext& context) {
    ...
    for (const auto cause : causes) {
        ...
        write_csr(state.csrs(),
                  rvemu::core::CsrAddress::Mideleg,
                  supervisor_cause ? cause_bit(cause_code) : 0U);
        ...
        if (expected_target == rvemu::core::PrivilegeMode::Supervisor) {
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Sepc) == interrupted_pc,
                           "S 中断必须写 sepc");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Scause) == encoded_cause,
                           "S 中断必须写 scause");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Stval) == 0U,
                           "中断 stval 必须为零");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mepc) == 0x3333'0000U,
                           "S 中断不得修改 mepc");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mcause) == 0x3333'0001U,
                           "S 中断不得修改 mcause");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mtval) == 0x3333'0002U,
                           "S 中断不得修改 mtval");
        } else {
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mepc) == interrupted_pc,
                           "M 中断必须写 mepc");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mcause) == encoded_cause,
                           "M 中断必须写 mcause");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Mtval) == 0U,
                           "中断 mtval 必须为零");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Sepc) == 0x4444'0000U,
                           "M 中断不得修改 sepc");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Scause) == 0x4444'0001U,
                           "M 中断不得修改 scause");
            context.expect(state.csrs().peek(rvemu::core::CsrAddress::Stval) == 0x4444'0002U,
                           "M 中断不得修改 stval");
        }
    }
}
...
void test_interrupt_priority_and_vectors(TestContext& context) {
    ...
    write_csr(priority_csrs,
              rvemu::core::CsrAddress::Mie,
              bit(1U) | bit(3U) | bit(5U) | bit(7U) | bit(9U) | bit(11U));
    for (const auto cause : priority) {
        priority_csrs.set_interrupt_pending(cause, true);
    }
    for (const auto expected : priority) {
        const auto selected =
            priority_csrs.select_pending_interrupt(rvemu::core::PrivilegeMode::User);
        context.expect(selected.has_value() && selected->cause == expected,
                       "中断必须遵循 MEI/MSI/MTI/SEI/SSI/STI 优先级");
    ...
    context.expect(machine_interrupt->target == rvemu::core::PrivilegeMode::Machine,
                   "MEI 必须进入 M-mode");
    context.expect(machine_state.program_counter() == kTrapBase + 44U,
                   "Vectored MEI 必须进入 BASE+4*11");
    context.expect(
        machine_state.csrs().peek(rvemu::core::CsrAddress::Mcause) == (kInterruptFlag | 11U),
        "mcause 必须包含中断最高位和 cause");
    context.expect(machine_state.csrs().peek(rvemu::core::CsrAddress::Mepc) == kRamBase + 0x100U,
                   "中断 mepc 必须保存下一条待执行 PC");
    ...
    supervisor_state.csrs().set_interrupt_pending(rvemu::core::InterruptCause::SupervisorExternal,
                                                  true);
    ...
    context.expect(supervisor_interrupt.has_value()
                       && supervisor_interrupt->target == rvemu::core::PrivilegeMode::Supervisor,
                   "U-mode 中已委托 SEI 必须进入 S-mode");
    context.expect(supervisor_state.program_counter() == kTrapBase + 36U,
                   "Vectored SEI 必须进入 BASE+4*9");
    context.expect(
        supervisor_state.csrs().peek(rvemu::core::CsrAddress::Scause) == (kInterruptFlag | 9U),
        "scause 必须记录监督级外部中断");

    supervisor_state.set_privilege(rvemu::core::PrivilegeMode::Machine);
    supervisor_state.csrs().set_interrupt_pending(rvemu::core::InterruptCause::SupervisorExternal,
                                                  true);
    context.expect(!supervisor_fixture.cpu().take_pending_interrupt().has_value(),
                   "已委托 S 中断在 M-mode 必须被屏蔽");
}
...
void test_privileged_legality_and_wfi(TestContext& context) {
    ...
    context.expect(result.stalled && !result.retired && !result.trap.has_value(),
                   "无局部使能中断时 WFI 必须保持等待");
    ...
    context.expect(!state.waiting_for_interrupt(),
                   "局部使能 pending 必须唤醒 WFI，即使全局 MIE 为零");
