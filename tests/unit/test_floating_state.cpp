...
void set_fs(rvemu::core::CsrFile& csrs, std::uint8_t state) {
    ...
    const auto desired = (current & ~(0x3ULL << 13U)) |
                         (static_cast<std::uint64_t>(state & 0x3U) << 13U);
...
void test_reset_gate_and_dirty_state(TestContext& context) {
    ...
    context.expect(read_csr(csrs, rvemu::core::CsrAddress::Fcsr) == 0U, "U-mode 应能读取已启用 fcsr");
    write_csr(
        csrs,
...
void test_fcsr_aliases(TestContext& context) {
    ...
    context.expect(read_csr(csrs, rvemu::core::CsrAddress::Fflags) == 0x05U, "fflags 必须投影 fcsr 低五位");
    context.expect(read_csr(csrs, rvemu::core::CsrAddress::Frm) == 0x05U, "frm 必须投影 fcsr 位 7:5");

    write_csr(csrs, rvemu::core::CsrAddress::Fflags, 0x3FU, rvemu::core::PrivilegeMode::User);
    context.expect(read_csr(csrs, rvemu::core::CsrAddress::Fflags) == 0x1FU, "fflags 写入必须屏蔽高位");
    ...
    context.expect(read_csr(csrs, rvemu::core::CsrAddress::Fflags) == 0x1FU, "写 frm 不得改变 fflags");
    context.expect(read_csr(csrs, rvemu::core::CsrAddress::Fcsr) == 0x7FU, "fcsr 必须立即反映两个别名视图");
}
...
void test_rounding_mode_resolution(TestContext& context) {
    ...
    for (std::uint8_t encoded = 0U; encoded < expected.size(); ++encoded) {
        ...
        context.expect(
            direct.has_value() && *direct == expected[encoded],
            "静态 rm=0..4 必须解析为对应舍入模式");
        const auto dynamic = rvemu::core::resolve_floating_rounding_mode(7U, encoded);
        context.expect(
            dynamic.has_value() && *dynamic == expected[encoded],
            "动态 rm 必须从合法 frm 解析对应舍入模式");
    }

    context.expect(
        !rvemu::core::resolve_floating_rounding_mode(5U, 0U).has_value(),
        "指令 rm=5 必须保留非法");
    context.expect(
        !rvemu::core::resolve_floating_rounding_mode(6U, 0U).has_value(),
        "指令 rm=6 必须保留非法");
    for (std::uint8_t dynamic = 5U; dynamic <= 7U; ++dynamic) {
        context.expect(
            !rvemu::core::resolve_floating_rounding_mode(7U, dynamic).has_value(),
            "动态 frm=5..7 必须解析失败");
    }
}
...
void test_exception_flag_accrual(TestContext& context) {
    ...
    context.expect(
        csrs.floating_exception_flags() == 0x15U,
...
void test_nan_boxing_and_cpu_state(TestContext& context) {
    ...
    context.expect(
        rvemu::core::box_single_precision(single_value) == 0xFFFF'FFFF'3F80'0000ULL,
        "单精度写入必须把上 32 位全部置一");
    context.expect(
        rvemu::core::unbox_single_precision(0xFFFF'FFFF'FF80'0000ULL) == 0xFF80'0000U,
        "合法 NaN box 必须原样返回低 32 位，包括负无穷位模式");
    context.expect(
        rvemu::core::unbox_single_precision(0x0000'0000'7F80'0001ULL) ==
    ...
    context.expect(
        state.floating(1U) == 0xFFFF'FFFF'3F80'0000ULL,
        "CpuState 单精度写入口必须保存合法 box");
    context.expect(!state.csrs().floating_state_enabled(), "内部寄存器写入不得从 FS=Off 偷偷启用状态");
    ...
    context.expect(
        ((state.csrs().peek(rvemu::core::CsrAddress::Mstatus) >> 13U) & 0x3U) == 0x3U,
        "已启用状态的浮点寄存器写入必须标记 FS Dirty");

    state.set_floating(3U, 0x0000'0000'7F80'0001ULL);
    context.expect(
        state.floating_single(3U) == rvemu::core::kCanonicalSingleNan,
        "CpuState 单精度读取必须统一处理非法 box");
    state.set_floating(4U, 0x0123'4567'89AB'CDEFULL);
    context.expect(
        state.floating(4U) == 0x0123'4567'89AB'CDEFULL,
        "双精度寄存器位模式不得经过单精度规范化");
