...
[[nodiscard]] constexpr std::uint32_t encode_r(std::uint32_t opcode,
    ...
    return ((function7 & 0x7FU) << 25U) | ((source2 & 0x1FU) << 20U) | ((source1 & 0x1FU) << 15U)
           | ((function3 & 0x07U) << 12U) | ((destination & 0x1FU) << 7U) | (opcode & 0x7FU);
}

[[nodiscard]] constexpr std::uint32_t encode_i(std::uint32_t opcode,
                                               std::uint32_t destination,
                                               std::uint32_t function3,
                                               std::uint32_t source1,
...
[[nodiscard]] constexpr std::uint32_t encode_s(std::uint32_t function3,
                                               std::uint32_t source1,
                                               std::uint32_t source2,
                                               std::uint32_t immediate12) noexcept {
...
[[nodiscard]] constexpr std::uint32_t encode_b(std::uint32_t function3,
                                               std::uint32_t source1,
                                               std::uint32_t source2,
    ...
    return (bit12 << 31U) | (bits10_5 << 25U) | ((source2 & 0x1FU) << 20U)
           | ((source1 & 0x1FU) << 15U) | ((function3 & 0x07U) << 12U) | (bits4_1 << 8U)
           | (bit11 << 7U) | 0x63U;
}

[[nodiscard]] constexpr std::uint32_t encode_u(std::uint32_t opcode,
                                               std::uint32_t destination,
                                               std::uint32_t upper20) noexcept {
    return ((upper20 & 0xFFFFFU) << 12U) | ((destination & 0x1FU) << 7U) | (opcode & 0x7FU);
}

[[nodiscard]] constexpr std::uint32_t encode_j(std::uint32_t destination,
    ...
    return (bit20 << 31U) | (bits10_1 << 21U) | (bit11 << 20U) | (bits19_12 << 12U)
           | ((destination & 0x1FU) << 7U) | 0x6FU;
}
   ...
   public:
    explicit CpuFixture(std::size_t ram_size = 0x2000U)
        : ram_(std::make_shared<rvemu::memory::PhysicalMemory>(
              rvemu::bus::PhysicalAddress{kRamBase}, ram_size, "cpu-test-ram")),
          cpu_(bus_) {
...
void test_architectural_state(TestContext& context) {
    ...
    context.expect(state.vector(31U)[0] == 0xAAU && state.vector(31U)[31] == 0x55U,
                   "向量寄存器必须保存 256 位状态");
...
void test_fetch_and_precise_faults(TestContext& context) {
    ...
    expect_trap(
        context, unmapped, rvemu::core::ExceptionCause::InstructionAccessFault, "未映射取指");
...
void test_upper_and_control_flow(TestContext& context) {
    ...
    context.expect(fixture.cpu().state().integer(1U) == 0xFFFF'FFFF'FFFF'F000ULL,
                   "LUI 必须符号扩展 32 位结果");

    result = fixture.execute(encode_u(0x17U, 2U, 0x1U));
    expect_retired(context, result, "AUIPC");
    context.expect(fixture.cpu().state().integer(2U) == kRamBase + 0x1000U,
                   "AUIPC 必须相对原 PC 计算");

    result = fixture.execute(encode_j(3U, 8U));
    expect_retired(context, result, "JAL");
    context.expect(fixture.cpu().state().integer(3U) == kRamBase + 4U,
                   "JAL 链接地址必须为下一指令");
    context.expect(fixture.cpu().state().program_counter() == kRamBase + 8U,
                   "JAL 必须跳转到 PC 相对目标");
...
void test_op_imm(TestContext& context) {
    ...
    const std::array<Case, 9U> cases{{
        ...
        {"SRAI",
         encode_i(0x13U, 2U, 5U, 1U, (0x10U << 6U) | 4U),
         0x8000'0000'0000'0000ULL,
         0xF800'0000'0000'0000ULL},
    }};
    ...
    for (const auto& item : cases) {
        ...
        context.expect(fixture.cpu().state().integer(2U) == item.expected,
                       std::string{item.name} + " 结果错误");
    }

    CpuFixture invalid;
    const auto invalid_shift = invalid.execute(encode_i(0x13U, 2U, 1U, 1U, (1U << 6U) | 1U));
    expect_trap(
        context, invalid_shift, rvemu::core::ExceptionCause::IllegalInstruction, "保留 SLLI 编码");
}
...
void test_op_register(TestContext& context) {
    ...
    for (const auto& item : cases) {
        ...
        expect_retired(context, result, item.name);
        context.expect(fixture.cpu().state().integer(3U) == item.expected,
                       std::string{item.name} + " 结果错误");
    }
}
...
void test_word_operations(TestContext& context) {
    ...
    const std::array<Case, 9U> cases{{
        ...
        {"SRAIW",
         encode_i(0x1BU, 3U, 5U, 1U, (0x20U << 5U) | 4U),
         0x8000'0000U,
         0U,
         0xFFFF'FFFF'F800'0000ULL},
        ...
        {"SRAW",
         encode_r(0x3BU, 3U, 5U, 1U, 2U, 0x20U),
         0x8000'0000U,
         4U,
         0xFFFF'FFFF'F800'0000ULL},
    }};
...
void test_branches(TestContext& context) {
    ...
    context.expect(not_taken.cpu().state().program_counter() == kRamBase + 4U,
                   "未采用分支必须顺序推进");
}
...
void test_loads_and_stores(TestContext& context) {
    ...
    const auto seed = fixture.bus().write(rvemu::bus::PhysicalAddress{data_address},
    ...
    for (const auto& item : stores) {
        ...
        expect_retired(context, result, item.name);
        const auto memory =
            fixture.bus().read(rvemu::bus::PhysicalAddress{data_address + item.offset},
                               item.width,
                               rvemu::bus::AccessType::Load);
        context.expect(memory.ok() && memory.value == item.expected,
                       std::string{item.name} + " 内存结果错误");
    }
}
...
void test_negative_immediates_aliasing_and_wraparound(TestContext& context) {
    ...
    context.expect(fixture.cpu().state().program_counter() == kRamBase,
                   "JAL -4 必须跳回前一条地址");
    ...
    context.expect(fixture.cpu().state().integer(6U) == 0x0123'4567'89AB'CDEFULL,
                   "LD -4 必须访问基址之前的数据");
    ...
    const auto stored = fixture.bus().read(rvemu::bus::PhysicalAddress{data_address},
                                           rvemu::bus::AccessWidth::DoubleWord,
                                           rvemu::bus::AccessType::Load);
    context.expect(stored.ok() && stored.value == 0xFEDC'BA98'7654'3210ULL,
                   "SD -4 必须写入基址之前的数据");
}
...
void test_fences_system_and_illegal(TestContext& context) {
    ...
    const auto seed = fixture.bus().write(rvemu::bus::PhysicalAddress{modified_instruction_address},
                                          rvemu::bus::AccessWidth::Word,
    ...
    context.expect(fixture.cpu().state().integer(24U) == 8U,
                   "FENCE.I 后必须执行被 SW 改写的新指令");
    ...
    expect_trap(
        context, result, rvemu::core::ExceptionCause::IllegalInstruction, "未实现 CSR 编码");
