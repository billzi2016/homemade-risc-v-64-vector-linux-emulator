...
[[nodiscard]] constexpr std::uint32_t encode_vsetvli(
    std::uint32_t destination,
    std::uint32_t source1,
    std::uint32_t vtype_immediate) noexcept {
    return ((vtype_immediate & 0x7FFU) << 20U) | ((source1 & 0x1FU) << 15U) |
           (0x7U << 12U) | ((destination & 0x1FU) << 7U) | 0x57U;
}
...
[[nodiscard]] constexpr std::uint32_t encode_vsetvl(
    std::uint32_t destination,
    std::uint32_t source1,
    std::uint32_t source2) noexcept {
    return (0x40U << 25U) | ((source2 & 0x1FU) << 20U) | ((source1 & 0x1FU) << 15U) |
           (0x7U << 12U) | ((destination & 0x1FU) << 7U) | 0x57U;
}
...
void test_three_vset_forms(TestContext& context) {
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vl) == 7U, "e32,m2 的 vl 必须取 AVL=7");
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vtype) == 0x11U, "vsetvli 必须提交合法 vtype");

    expect_retired(context, fixture.execute(encode_vsetivli(6U, 31U, 0x18U)), "vsetivli e64,m1");
    context.expect(fixture.cpu().state().integer(6U) == 4U, "e64,m1 的 VLMAX=4，过大 AVL 必须选择 VLMAX");
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vl) == 4U, "vsetivli 必须更新 vl");

    fixture.cpu().state().set_integer(2U, 0x5U);
    expect_retired(context, fixture.execute(encode_vsetvl(7U, 0U, 2U)), "vsetvl e8,mf8");
    context.expect(fixture.cpu().state().integer(7U) == 4U, "rs1=x0 且 rd 非零时 AVL 应为全一并取 mf8 的 VLMAX=4");
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vtype) == 0x5U, "vsetvl 必须读取 rs2 的完整 vtype");
}
...
void test_illegal_and_preserve_rules(TestContext& context) {
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vl) == 3U, "初始 vl 必须为 3");
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vl) == 3U, "保持形式不得重算或清零 vl");
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vstart) == 0U, "成功 vset* 必须清零 vstart");

    expect_illegal(context, fixture.execute(encode_vsetvli(0U, 0U, 0x10U)), "改变 VLMAX 的保持形式");
    ...
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vtype) == kVill, "非法 vtype 必须提交仅置 vill 的 vtype");
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vl) == 0U, "非法 vtype 必须提交 vl=0");

    expect_retired(context, fixture.execute(encode_vsetvli(9U, 1U, 0x100U)), "保留高位的 vsetvli");
    context.expect(fixture.cpu().state().csrs().peek(rvemu::core::CsrAddress::Vtype) == kVill, "非零保留高位同样必须提交 vill");
}
