...
public:
    ...
    void write_data(std::uint64_t address, bool word_operation, std::uint64_t value) {
        const auto width = word_operation ? rvemu::bus::AccessWidth::Word :
...
void test_lr_sc_basic(TestContext& context) {
    ...
    context.expect(
        word.cpu().state().integer(3U) == 0xFFFF'FFFF'8000'0001ULL,
        "LR.W 必须符号扩展旧值");
    ...
    context.expect(word.read_data(kDataAddress, true) == 0xCAFE'BABEU, "成功 SC.W 必须提交低 32 位");
    ...
    context.expect(
        double_word.cpu().state().integer(5U) == 0x0123'4567'89AB'CDEFULL,
        "LR.D 必须返回完整 64 位旧值");
...
void test_reservation_invalidation(TestContext& context) {
    ...
    context.expect(
        fixture.bus().write(
            rvemu::bus::PhysicalAddress{kDataAddress + 0x20U},
            rvemu::bus::AccessWidth::Word,
            0xAAU,
    ...
    context.expect(
        fixture.bus().write(
            rvemu::bus::PhysicalAddress{kDataAddress + 3U},
...
constexpr std::array<AmoCase, 9U> kDoubleCases{{
    ...
    {"AMOXOR.D", 0x04U, 0xA5A5'0000'FFFF'0000ULL, 0x00FF'00FF'00FF'00FFULL, 0xA55A'00FF'FF00'00FFULL},
    {"AMOAND.D", 0x0CU, 0xF0F0'0F0F'F0F0'0F0FULL, 0x0FF0'FFF0'0FF0'FFF0ULL, 0x00F0'0F00'00F0'0F00ULL},
    {"AMOOR.D", 0x08U, 0xF000'000F'F000'000FULL, 0x0FF0'00F0'0FF0'00F0ULL, 0xFFF0'00FF'FFF0'00FFULL},
...
void execute_amo_cases(
    ...
    for (const auto& test_case : cases) {
        ...
        const auto expected_old = word_operation && (test_case.observed & 0x8000'0000U) != 0U ?
                                      ...
                                      test_case.observed;
        context.expect(
            fixture.cpu().state().integer(3U) == expected_old,
            std::string{test_case.name} + " rd 旧值错误");
        context.expect(
            fixture.read_data(kDataAddress, word_operation) == test_case.expected_memory,
...
void test_illegal_fault_and_alignment(TestContext& context) {
    ...
    auto result = fixture.execute(encode_atomic(0x02U, false, 3U, 1U, 0U));
    expect_trap(
        context, result, rvemu::core::ExceptionCause::LoadAddressMisaligned,
        kDataAddress + 1U, "错位 LR.D");
    result = fixture.execute(encode_atomic(0x03U, false, 3U, 1U, 2U));
    expect_trap(
        context, result, rvemu::core::ExceptionCause::StoreAddressMisaligned,
        kDataAddress + 1U, "错位 SC.D");
    result = fixture.execute(encode_atomic(0x00U, false, 3U, 1U, 2U));
    expect_trap(
        context, result, rvemu::core::ExceptionCause::StoreAddressMisaligned,
        kDataAddress + 1U, "错位 AMOADD.D");
    context.expect(
        fixture.read_data(kDataAddress, false) == 0x1122'3344'5566'7788ULL,
        "错位原子指令不得产生部分内存副作用");
    ...
    expect_trap(
        context, result, rvemu::core::ExceptionCause::LoadAccessFault,
        0x7000'0000U, "未映射 LR.D");
    result = unmapped.execute(encode_atomic(0x00U, false, 3U, 1U, 2U));
    expect_trap(
        context, result, rvemu::core::ExceptionCause::StoreAccessFault,
        0x7000'0000U, "未映射 AMOADD.D");

    CpuFixture illegal;
    illegal.cpu().state().set_integer(1U, kDataAddress);
    result = illegal.execute(encode_atomic(0x02U, false, 3U, 1U, 2U));
    expect_trap(
        context, result, rvemu::core::ExceptionCause::IllegalInstruction,
        encode_atomic(0x02U, false, 3U, 1U, 2U), "LR rs2 非零");
    result = illegal.execute(encode_atomic(0x05U, false, 3U, 1U, 2U));
    expect_trap(
        context, result, rvemu::core::ExceptionCause::IllegalInstruction,
        encode_atomic(0x05U, false, 3U, 1U, 2U), "保留 AMO funct5");

    // funct3=001 不是 A 扩展合法宽度；直接修改真实编码以验证非法分支。
    const auto invalid_width = (encode_atomic(0x00U, true, 3U, 1U, 2U) & ~(0x7U << 12U)) |
                               (0x1U << 12U);
    result = illegal.execute(invalid_width);
    expect_trap(
        context, result, rvemu::core::ExceptionCause::IllegalInstruction,
        invalid_width, "保留 AMO 宽度");
}
