...
void test_address_ranges(TestContext& context) {
    ...
    if (valid.has_value()) {
        ...
        context.expect(
            !valid->contains(PhysicalAddress{0x10FFU}, 2U),
            "跨越区间末端的多字节访问应被拒绝");
    }

    context.expect(
        !rvemu::bus::AddressRange::create(PhysicalAddress{0x1000U}, 0U).has_value(),
        "零长度区域应被拒绝");
    context.expect(
        !rvemu::bus::AddressRange::create(
             PhysicalAddress{std::numeric_limits<std::uint64_t>::max() - 3U},
             8U)
             .has_value(),
        "结束地址溢出的区域应被拒绝");
}
...
void test_fixed_address_map(TestContext& context) {
    ...
    for (std::size_t index = 0U; index < entries.size(); ++index) {
        const auto current = rvemu::bus::AddressRange::create(
            PhysicalAddress{entries[index].base},
            entries[index].size);
        context.expect(current.has_value(), std::string{entries[index].name} + " 固定地址范围必须合法");
        ...
        for (std::size_t other_index = index + 1U; other_index < entries.size(); ++other_index) {
            const auto other = rvemu::bus::AddressRange::create(
                PhysicalAddress{entries[other_index].base},
                entries[other_index].size);
            context.expect(other.has_value(), "所有固定地址范围必须可构造");
            if (other.has_value()) {
                context.expect(
                    !current->overlaps(*other),
                    std::string{entries[index].name} + " 不得与 " + entries[other_index].name + " 重叠");
            }
        }
    }
}
...
void test_bus_registration(TestContext& context) {
    ...
    context.expect(
        null_registration.error.code == BusErrorCode::InvalidRegion,
        "空区域注册必须返回 InvalidRegion");

    auto ram = std::make_shared<rvemu::memory::PhysicalMemory>(
        PhysicalAddress{0x8000'0000U},
        0x1000U,
        "test-ram");
    auto rom = std::make_shared<rvemu::memory::BootRom>(
        PhysicalAddress{0x0000'1000U},
        0x100U,
        "test-rom");
...
void test_ram_endianness_and_widths(TestContext& context) {
    ...
    const auto write64 = bus.write(
        PhysicalAddress{base + 3U},
    ...
    const auto full = bus.read(PhysicalAddress{base + 3U}, AccessWidth::DoubleWord, AccessType::Load);
    ...
    context.expect(
        ...
        "8 位写入应成功");
    context.expect(
        bus.write(PhysicalAddress{base + 0x22U}, AccessWidth::HalfWord, 0x1'BEEFU, AccessType::Store).ok(),
        "16 位写入应成功");
    context.expect(
        bus.write(PhysicalAddress{base + 0x24U}, AccessWidth::Word, 0x1'89AB'CDEFULL, AccessType::Store).ok(),
        "32 位写入应成功");
    context.expect(
        bus.write(
            PhysicalAddress{base + 0x28U},
            AccessWidth::DoubleWord,
    ...
    const auto byte_written = bus.read(PhysicalAddress{base + 0x20U}, AccessWidth::Byte, AccessType::Load);
    const auto half_written = bus.read(PhysicalAddress{base + 0x22U}, AccessWidth::HalfWord, AccessType::Load);
    const auto word_written = bus.read(PhysicalAddress{base + 0x24U}, AccessWidth::Word, AccessType::Load);
    const auto double_written = bus.read(
        PhysicalAddress{base + 0x28U},
        AccessWidth::DoubleWord,
        AccessType::Load);
    ...
    context.expect(word_written.ok() && word_written.value == 0x89AB'CDEFU, "32 位写入必须截断高位");
    context.expect(
        double_written.ok() && double_written.value == 0xFEDC'BA98'7654'3210ULL,
        "64 位写入必须完整保留");
}
...
void test_bus_failures_have_no_partial_effect(TestContext& context) {
    ...
    const auto sentinel = bus.write(
        PhysicalAddress{base + size - 1U},
        AccessWidth::Byte,
        0xA5U,
        AccessType::Store);
    ...
    const auto unchanged = bus.read(
        PhysicalAddress{base + size - 1U},
        AccessWidth::Byte,
        AccessType::Load);
    context.expect(unchanged.ok() && unchanged.value == 0xA5U, "跨界失败不得修改首字节");

    const auto unmapped = bus.read(PhysicalAddress{0x7000'0000U}, AccessWidth::Word, AccessType::Load);
    context.expect(!unmapped.ok() && unmapped.error.code == BusErrorCode::Unmapped, "未映射读取必须结构化失败");

    const auto overflow = bus.read(
        PhysicalAddress{std::numeric_limits<std::uint64_t>::max()},
        AccessWidth::HalfWord,
        AccessType::Load);
    context.expect(
        !overflow.ok() && overflow.error.code == BusErrorCode::AddressOverflow,
...
void test_boot_rom_lifecycle(TestContext& context) {
    ...
    context.expect(
        !oversized_load.ok() && oversized_load.error.code == BusErrorCode::OutOfBounds,
    ...
    const auto instruction = bus.read(PhysicalAddress{base}, AccessWidth::Word, AccessType::InstructionFetch);
    context.expect(instruction.ok() && instruction.value == 0x13U, "Boot ROM 指令读取必须为小端值");

    const auto runtime_write = bus.write(
        PhysicalAddress{base},
        AccessWidth::Word,
        0xFFFF'FFFFU,
        AccessType::Store);
    context.expect(
        !runtime_write.ok() && runtime_write.error.code == BusErrorCode::ReadOnly,
        "运行期总线写入 Boot ROM 必须失败");

    const auto late_load = rom->load(0U, image);
    context.expect(
        !late_load.ok() && late_load.error.code == BusErrorCode::ReadOnly,
...
void test_atomic_compare_exchange(TestContext& context) {
    ...
    context.expect(
        bus.write(PhysicalAddress{base + 8U}, AccessWidth::DoubleWord, 0x10U, AccessType::Store).ok(),
        "原子测试初值写入必须成功");

    const auto success = bus.compare_exchange(
        PhysicalAddress{base + 8U},
        AccessWidth::DoubleWord,
        0x10U,
        0x20U);
    ...
    const auto failure = bus.compare_exchange(
        PhysicalAddress{base + 8U},
        AccessWidth::DoubleWord,
        0x10U,
        0x30U);
    ...
    context.expect(
        bus.write(PhysicalAddress{base + 0x20U}, AccessWidth::Word, 0xFFFF'FF80U, AccessType::Store).ok(),
        "32 位原子测试初值写入必须成功");
    const auto narrow = bus.compare_exchange(
        PhysicalAddress{base + 0x20U},
        AccessWidth::Word,
        0xCAFE'BABE'FFFF'FF80ULL,
        0x1234'5678'89AB'CDEFULL);
    context.expect(narrow.ok() && narrow.exchanged, "窄原子事务只比较访问宽度内的低位");
    const auto narrow_value = bus.read(
        PhysicalAddress{base + 0x20U},
        AccessWidth::Word,
        AccessType::Load);
    context.expect(
        narrow_value.ok() && narrow_value.value == 0x89AB'CDEFU,
        "窄原子事务只提交访问宽度内的低位");

    const auto crossing = bus.compare_exchange(
        PhysicalAddress{base + 0xFEU},
        AccessWidth::Word,
        0U,
        1U);
    context.expect(
        !crossing.ok() && crossing.error.code == BusErrorCode::OutOfBounds,
