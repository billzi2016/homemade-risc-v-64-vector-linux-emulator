...
struct Fixture final {
    ...
    std::shared_ptr<rvemu::memory::PhysicalMemory> ram{
        std::make_shared<rvemu::memory::PhysicalMemory>(
            rvemu::bus::PhysicalAddress{kRamBase}, static_cast<std::size_t>(kRamSize), "ram")};
    ...
    void write64(std::uint64_t address, std::uint64_t value) {
        const auto written = bus.write(rvemu::bus::PhysicalAddress{address},
    ...
    [[nodiscard]] std::uint64_t read64(std::uint64_t address) {
        const auto read = bus.read(rvemu::bus::PhysicalAddress{address},
                                   rvemu::bus::AccessWidth::DoubleWord,
...
[[nodiscard]] constexpr std::uint64_t pte(std::uint64_t physical_address,
...
[[nodiscard]] rvemu::memory::MmuContext supervisor_context(std::uint64_t status = 0U) noexcept {
    return rvemu::memory::MmuContext{rvemu::core::PrivilegeMode::Supervisor, satp(kRoot), status};
}

[[nodiscard]] rvemu::memory::MmuContext user_context(std::uint64_t status = 0U) noexcept {
    return rvemu::memory::MmuContext{rvemu::core::PrivilegeMode::User, satp(kRoot), status};
}

void install_three_level_mapping(Fixture& fixture,
...
void test_bare_and_canonical(TestContext& context) {
    ...
    context.expect(bare.physical_address.has_value() && bare.physical_address->value() == 0x1234U
                       && !bare.fault.has_value(),
                   "satp Bare 必须直通物理地址");

    const auto bad = fixture.mmu.translate(1ULL << 39U,
                                           rvemu::bus::AccessWidth::Word,
                                           rvemu::memory::MmuAccessKind::Load,
                                           supervisor_context());
    context.expect(!bad.physical_address.has_value() && bad.fault.has_value()
...
void test_leaf_ad_and_permissions(TestContext& context) {
    ...
    const auto loaded = fixture.mmu.translate(0x4008U,
                                              rvemu::bus::AccessWidth::DoubleWord,
                                              rvemu::memory::MmuAccessKind::Load,
                                              user_context());
    context.expect(
        loaded.physical_address.has_value() && loaded.physical_address->value() == kPage4K + 8U,
        "4KiB 叶子必须合成 PPN 和页内偏移");
    const auto after_load = fixture.read64(kLevel0 + 4U * 8U);
    context.expect((after_load & kPteA) != 0U && (after_load & kPteD) == 0U,
                   "加载成功后必须只原子设置 A 位");

    const auto stored = fixture.mmu.translate(0x4010U,
                                              rvemu::bus::AccessWidth::Word,
                                              rvemu::memory::MmuAccessKind::Store,
                                              user_context());
    context.expect(
        stored.physical_address.has_value() && stored.physical_address->value() == kPage4K + 0x10U,
        "存储翻译必须复用同一 4KiB 映射");
    const auto after_store = fixture.read64(kLevel0 + 4U * 8U);
    context.expect((after_store & kPteA) != 0U && (after_store & kPteD) != 0U,
                   "存储成功后必须同时保证 A/D 位已设置");

    Fixture denied{};
    install_three_level_mapping(denied, kPteV | kPteR | kPteW);
    const auto user_denied = denied.mmu.translate(
        0x4000U, rvemu::bus::AccessWidth::Word, rvemu::memory::MmuAccessKind::Load, user_context());
    context.expect(user_denied.fault.has_value()
                       && user_denied.fault->cause == rvemu::core::ExceptionCause::LoadPageFault,
                   "U-mode 访问 U=0 内核页必须页错误");
    context.expect((denied.read64(kLevel0 + 4U * 8U) & (kPteA | kPteD)) == 0U,
                   "权限拒绝不得提前设置 A/D 位");

    Fixture atomic_denied{};
    install_three_level_mapping(atomic_denied, kPteV | kPteW | kPteU | kPteA | kPteD);
    const auto atomic = atomic_denied.mmu.translate(0x4000U,
                                                    rvemu::bus::AccessWidth::Word,
                                                    rvemu::memory::MmuAccessKind::Atomic,
                                                    user_context());
    context.expect(atomic.fault.has_value()
...
void test_superpages_and_mxr_sum(TestContext& context) {
    Fixture fixture{};
    fixture.write64(kRoot + 0U * 8U, pte(kLevel1, kPteV));
    fixture.write64(kLevel1 + 1U * 8U,
                    pte(kRamBase + 0x20'0000U, kPteV | kPteR | kPteW | kPteA | kPteD));
    const auto two_mib = fixture.mmu.translate(0x20'1234U,
                                               rvemu::bus::AccessWidth::Word,
                                               rvemu::memory::MmuAccessKind::Load,
                                               supervisor_context());
    context.expect(two_mib.physical_address.has_value()
    ...
    const auto one_gib = fixture.mmu.translate(0x4000'4567U,
                                               rvemu::bus::AccessWidth::Word,
                                               rvemu::memory::MmuAccessKind::Load,
                                               supervisor_context());
    context.expect(one_gib.physical_address.has_value()
    ...
    const auto bad_2m = misaligned.mmu.translate(0x20'0000U,
                                                 rvemu::bus::AccessWidth::Word,
                                                 rvemu::memory::MmuAccessKind::Load,
                                                 supervisor_context());
    context.expect(bad_2m.fault.has_value()
    ...
    const auto bad_1g = misaligned.mmu.translate(0x4000'0000U,
                                                 rvemu::bus::AccessWidth::Word,
                                                 rvemu::memory::MmuAccessKind::Load,
                                                 supervisor_context());
    context.expect(bad_1g.fault.has_value()
    ...
    const auto no_mxr = mxr.mmu.translate(
        0x4000U, rvemu::bus::AccessWidth::Word, rvemu::memory::MmuAccessKind::Load, user_context());
    const auto with_mxr = mxr.mmu.translate(0x4000U,
                                            rvemu::bus::AccessWidth::Word,
                                            rvemu::memory::MmuAccessKind::Load,
    ...
    const auto no_sum = sum.mmu.translate(0x4000U,
                                          rvemu::bus::AccessWidth::Word,
                                          rvemu::memory::MmuAccessKind::Load,
                                          supervisor_context());
    const auto with_sum = sum.mmu.translate(0x4000U,
                                            rvemu::bus::AccessWidth::Word,
                                            rvemu::memory::MmuAccessKind::Load,
...
void test_tlb_and_sfence(TestContext& context) {
    ...
    const auto first = fixture.mmu.translate(0x4000U,
                                             rvemu::bus::AccessWidth::Word,
                                             rvemu::memory::MmuAccessKind::Load,
                                             supervisor_context());
    ...
    const auto cached = fixture.mmu.translate(0x4000U,
                                              rvemu::bus::AccessWidth::Word,
                                              rvemu::memory::MmuAccessKind::Load,
                                              supervisor_context());
    context.expect(
        cached.physical_address.has_value() && cached.physical_address->value() == kPage4K,
        "未执行 SFENCE.VMA 时允许命中旧 TLB 翻译");

    fixture.mmu.sfence_vma(0x4000U, std::nullopt);
    const auto refilled = fixture.mmu.translate(0x4000U,
                                                rvemu::bus::AccessWidth::Word,
                                                rvemu::memory::MmuAccessKind::Load,
                                                supervisor_context());
    context.expect(refilled.physical_address.has_value()
...
void test_page_fault_causes(TestContext& context) {
    ...
    const auto instruction_fault =
        fixture.mmu.translate(0x4000U,
                              rvemu::bus::AccessWidth::HalfWord,
                              rvemu::memory::MmuAccessKind::InstructionFetch,
                              supervisor_context());
    const auto load_fault = fixture.mmu.translate(0x5000U,
                                                  rvemu::bus::AccessWidth::Word,
                                                  rvemu::memory::MmuAccessKind::Load,
                                                  supervisor_context());
    const auto store_fault = fixture.mmu.translate(0x4000U,
                                                   rvemu::bus::AccessWidth::Word,
                                                   rvemu::memory::MmuAccessKind::Store,
                                                   supervisor_context());
    ...
    context.expect(load_fault.fault.has_value()
    ...
    context.expect(store_fault.fault.has_value()
