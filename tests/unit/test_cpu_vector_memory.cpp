...
[[nodiscard]] constexpr std::uint32_t encode_vsetivli(std::uint32_t avl,
...
[[nodiscard]] constexpr std::uint32_t encode_vector_memory(bool load,
                                                           std::uint32_t width,
    ...
    return ((mode & 0x3U) << 26U) | ((unmasked ? 1U : 0U) << 25U)
           ...
           | ((width & 0x7U) << 12U) | ((vector_register & 0x1FU) << 7U) | (load ? 0x07U : 0x27U);
}
...
class CpuFixture final {
   public:
    CpuFixture()
        ...
        const auto enabled = cpu_.state().csrs().access(
            rvemu::core::CsrAccessRequest{rvemu::core::CsrAddress::Mstatus,
    ...
    [[nodiscard]] std::uint8_t read_byte(std::uint64_t address) {
        const auto result = bus_.read(rvemu::bus::PhysicalAddress{address},
                                      rvemu::bus::AccessWidth::Byte,
...
[[nodiscard]] rvemu::vector::VectorRegisterGroup require_group(rvemu::core::Cpu& cpu,
...
void test_unit_stride_unaligned_load(TestContext& context) {
    ...
    expect_retired(context,
                   fixture.execute(encode_vector_memory(true, 5U, 2U, 1U, 0U, 0U, true)),
                   "非对齐 vle16");
...
void test_strided_store(TestContext& context) {
    ...
    expect_retired(context,
                   fixture.execute(encode_vector_memory(false, 0U, 4U, 1U, 2U, 2U, true)),
                   "负 stride vse8");
