...
// 边界：本文件不修改 VS/CSR；直接使用 VectorState 仅服务于纯布局层，CPU 提交必须经 CpuState
// 入口标记 Dirty。
...
std::optional<std::uint64_t> VectorRegisterGroup::read_element(const VectorState& state,
...
bool VectorRegisterGroup::write_element(VectorState& state,
    ...
    for (std::uint8_t byte = 0U; byte < location->width_bytes; ++byte) {
        state.set_byte_value(location->register_index,
...
std::optional<bool> VectorRegisterGroup::mask_bit(const VectorState& state,
