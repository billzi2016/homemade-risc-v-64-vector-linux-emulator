...
[[nodiscard]] constexpr UnsignedProduct128 multiply_unsigned_64(
    std::uint64_t lhs,
    ...
    const auto middle = (low_low >> 32U) + (low_high & limb_mask) +
                        (high_low & limb_mask);
    const auto low = (low_low & limb_mask) | (middle << 32U);
    const auto high = high_high + (low_high >> 32U) + (high_low >> 32U) +
                      (middle >> 32U);
...
template <typename UInt>
[[nodiscard]] constexpr SignedDivisionResult<UInt> divide_signed(
    UInt dividend,
    UInt divisor) noexcept {
    static_assert(std::is_unsigned_v<UInt>, "有符号除法位模式必须由无符号类型承载");
    constexpr auto sign_bit = static_cast<UInt>(
        static_cast<UInt>(1U) << (std::numeric_limits<UInt>::digits - 1U));
    ...
    const auto dividend_magnitude = dividend_negative ?
                                        twos_complement_negate(dividend) : dividend;
    const auto divisor_magnitude = divisor_negative ?
                                       twos_complement_negate(divisor) : divisor;
...
[[nodiscard]] std::optional<std::uint64_t> execute_doubleword(
...
[[nodiscard]] std::optional<std::uint64_t> execute_word(
    std::uint8_t function3,
    std::uint64_t lhs,
    std::uint64_t rhs) noexcept {
    ...
    case 5U: {  // DIVUW
        const auto quotient = rhs_word == 0U ?
    ...
    case 7U: {  // REMUW
        const auto remainder = rhs_word == 0U ? lhs_word :
                               static_cast<std::uint32_t>(lhs_word % rhs_word);
...
std::optional<std::uint64_t> execute_integer_m(
    std::uint8_t function3,
    std::uint64_t lhs,
    std::uint64_t rhs,
