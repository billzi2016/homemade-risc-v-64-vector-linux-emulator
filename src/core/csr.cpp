...
constexpr std::uint64_t kSstatusWriteMask =
    bit(1U) | bit(5U) | bit(8U) | (0x3ULL << 9U) | (0x3ULL << 13U) | bit(18U) | bit(19U);
constexpr std::uint64_t kSstatusReadMask = kSstatusWriteMask | (0x3ULL << 32U) | bit(63U);
constexpr std::uint64_t kMstatusWriteMask = kSstatusWriteMask | bit(3U) | bit(7U) | (0x3ULL << 11U)
...
constexpr std::uint64_t kDelegatableExceptionMask = bit(0U) | bit(1U) | bit(2U) | bit(3U) | bit(4U)
                                                    ...
                                                    | bit(9U) | bit(12U) | bit(13U) | bit(15U);

constexpr std::uint64_t kMisa =
    (0x2ULL << 62U) | bit(static_cast<std::uint8_t>('A' - 'A'))
    | bit(static_cast<std::uint8_t>('C' - 'A')) | bit(static_cast<std::uint8_t>('D' - 'A'))
    | bit(static_cast<std::uint8_t>('F' - 'A')) | bit(static_cast<std::uint8_t>('I' - 'A'))
...
bool CsrFile::access_allowed(CsrAddress address,
    ...
    if (address == CsrAddress::Cycle || address == CsrAddress::Time
        ...
        if (privilege == PrivilegeMode::Supervisor && (mcounteren_ & counter_mask) == 0U) {
            return false;
        }
        if (privilege == PrivilegeMode::User
            && ((mcounteren_ & counter_mask) == 0U || (scounteren_ & counter_mask) == 0U)) {
            return false;
        }
    }
...
void CsrFile::commit_vector_configuration(std::uint64_t vtype, std::uint64_t vl) noexcept {
    // 此入口不负责重新解释 vtype，避免 CSR 与 CPU
    // 各自形成一套能力表；调用方已使用唯一配置模块归一。
    vtype_ = vtype;
    vl_ = vl;
...
void CsrFile::write_value(CsrAddress address, std::uint64_t value) noexcept {
    switch (address) {
        case CsrAddress::Fflags:
            ...
            break;
        case CsrAddress::Frm:
            fcsr_ = static_cast<std::uint8_t>((fcsr_ & kFloatingExceptionMask)
                                              | ((value & 0x7U) << 5U));
            mark_floating_state_dirty();
            break;
        case CsrAddress::Fcsr:
        ...
        case CsrAddress::Mtval:
            ...
            break;
        case CsrAddress::Mip:
            mip_ = (mip_ & ~kSoftwareWritablePendingMask) | (value & kSoftwareWritablePendingMask);
            break;
        case CsrAddress::Mcycle:
            cycle_ = value;
            break;
...
std::optional<PendingInterrupt> CsrFile::select_pending_interrupt(
    ...
    for (const auto cause : priority) {
        ...
        if (delegated) {
            ...
            const auto globally_enabled =
                current_privilege == PrivilegeMode::User || (mstatus_ & bit(1U)) != 0U;
            if (globally_enabled) {
        ...
        const auto globally_enabled =
            current_privilege != PrivilegeMode::Machine || (mstatus_ & bit(3U)) != 0U;
        if (globally_enabled) {
...
void CsrFile::enter_machine_trap(PrivilegeMode previous_privilege,
...
void CsrFile::enter_supervisor_trap(PrivilegeMode previous_privilege,
                                    std::uint64_t program_counter,
                                    std::uint64_t cause,
                                    std::uint64_t trap_value) noexcept {
...
TrapReturnState CsrFile::return_from_supervisor() noexcept {
    const auto target =
        (mstatus_ & bit(8U)) != 0U ? PrivilegeMode::Supervisor : PrivilegeMode::User;
...
std::uint64_t CsrFile::trap_vector(PrivilegeMode target,
                                   bool interrupt,
...
bool CsrFile::exception_delegated(PrivilegeMode source, ExceptionCause cause) const noexcept {
