...
[[nodiscard]] FloatingIntegerResult invalid_integer_result(bool sign,
    ...
    return FloatingIntegerResult{
        value, static_cast<std::uint8_t>(FloatingExceptionFlag::InvalidOperation)};
}

}  // namespace

FloatingResult floating_convert_format(FloatingFormat destination,
    ...
    if (soft_float_detail::is_nan(operand.category)) {
        return FloatingResult{target.canonical_nan,
    ...
    return soft_float_detail::round_and_pack(
        target, operand.sign, WideUnsigned{operand.significand}, operand.exponent, rounding);
}

FloatingResult floating_from_integer(FloatingFormat destination,
    ...
    if (!unsigned_integer) {
        const auto sign_bit = integer_width == 32U ? 0x8000'0000ULL : 0x8000'0000'0000'0000ULL;
    ...
    return soft_float_detail::round_and_pack(
        soft_float_detail::format_of(destination), sign, WideUnsigned{magnitude}, 0, rounding);
}

FloatingIntegerResult floating_to_integer(FloatingFormat source,
                                          std::uint64_t operand_bits,
                                          bool unsigned_integer,
                                          std::uint8_t integer_width,
                                          FloatingRoundingMode rounding) noexcept {
    ...
    const auto magnitude = soft_float_detail::round_to_integer_magnitude(
        WideUnsigned{operand.significand}, operand.exponent, operand.sign, rounding, inexact);
    ...
    if ((operand.sign && rounded > negative_limit) || (!operand.sign && rounded > positive_limit)) {
