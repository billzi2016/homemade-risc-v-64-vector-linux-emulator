...
[[nodiscard]] constexpr std::uint8_t full_register(std::uint16_t bits,
                                                   std::uint8_t shift) noexcept {
    return static_cast<std::uint8_t>((bits >> shift) & 0x1FU);
}

[[nodiscard]] constexpr std::uint32_t encode_i(std::uint8_t opcode,
...
[[nodiscard]] constexpr std::uint32_t encode_r(std::uint8_t opcode,
                                               std::uint8_t destination,
                                               std::uint8_t function3,
                                               std::uint8_t source1,
                                               std::uint8_t source2,
...
[[nodiscard]] constexpr std::uint32_t encode_s(std::uint8_t opcode,
                                               std::uint8_t function3,
                                               std::uint8_t source1,
                                               std::uint8_t source2,
                                               std::uint64_t immediate) noexcept {
    const auto encoded = static_cast<std::uint32_t>(immediate & 0xFFFU);
    return ((encoded >> 5U) << 25U) | (static_cast<std::uint32_t>(source2) << 20U)
           | (static_cast<std::uint32_t>(source1) << 15U)
           | (static_cast<std::uint32_t>(function3) << 12U) | ((encoded & 0x1FU) << 7U) | opcode;
}

[[nodiscard]] constexpr std::uint32_t encode_b(std::uint8_t function3,
                                               std::uint8_t source1,
                                               std::uint8_t source2,
                                               std::uint64_t immediate) noexcept {
    const auto encoded = static_cast<std::uint32_t>(immediate & 0x1FFFU);
    return (((encoded >> 12U) & 1U) << 31U) | (((encoded >> 5U) & 0x3FU) << 25U)
           | (static_cast<std::uint32_t>(source2) << 20U)
           | (static_cast<std::uint32_t>(source1) << 15U)
           | (static_cast<std::uint32_t>(function3) << 12U) | (((encoded >> 1U) & 0x0FU) << 8U)
...
[[nodiscard]] constexpr std::uint32_t encode_j(std::uint64_t immediate) noexcept {
    ...
    return (((encoded >> 20U) & 1U) << 31U) | (((encoded >> 1U) & 0x3FFU) << 21U)
           | (((encoded >> 11U) & 1U) << 20U) | (((encoded >> 12U) & 0xFFU) << 12U) | 0x6FU;
}

[[nodiscard]] std::uint64_t ci_immediate(std::uint16_t bits) noexcept {
    const auto encoded =
        static_cast<std::uint64_t>(((bits >> 7U) & 0x20U) | ((bits >> 2U) & 0x1FU));
    return sign_extend(encoded, 6U);
}

[[nodiscard]] std::uint64_t jump_immediate(std::uint16_t bits) noexcept {
    const auto encoded = (static_cast<std::uint64_t>((bits >> 12U) & 1U) << 11U)
...
[[nodiscard]] std::uint64_t branch_immediate(std::uint16_t bits) noexcept {
    const auto encoded = (static_cast<std::uint64_t>((bits >> 12U) & 1U) << 8U)
...
[[nodiscard]] std::optional<std::uint32_t> decompress_quadrant0(std::uint16_t bits,
    ...
    if (function3 == 0U) {  // C.ADDI4SPN
        const auto immediate = (static_cast<std::uint64_t>((bits >> 7U) & 0xFU) << 6U)
    ...
    if (function3 == 1U || function3 == 3U) {  // C.FLD / C.LD
        const auto immediate = (static_cast<std::uint64_t>((bits >> 5U) & 0x3U) << 6U)
                               | (static_cast<std::uint64_t>((bits >> 10U) & 0x7U) << 3U);
        return encode_i(function3 == 1U ? 0x07U : 0x03U, destination, 3U, source1, immediate);
    }
    if (function3 == 2U) {  // C.LW
        const auto immediate = (static_cast<std::uint64_t>((bits >> 5U) & 1U) << 6U)
                               | (static_cast<std::uint64_t>((bits >> 10U) & 0x7U) << 3U)
                               | (static_cast<std::uint64_t>((bits >> 6U) & 1U) << 2U);
    ...
    if (function3 == 5U || function3 == 7U) {  // C.FSD / C.SD
        const auto immediate = (static_cast<std::uint64_t>((bits >> 5U) & 0x3U) << 6U)
                               | (static_cast<std::uint64_t>((bits >> 10U) & 0x7U) << 3U);
        return encode_s(function3 == 5U ? 0x27U : 0x23U, 3U, source1, source2, immediate);
    }
...
[[nodiscard]] std::optional<std::uint32_t> decompress_misc_alu(std::uint16_t bits) noexcept {
    ...
    return encode_r(0x3BU, destination, 0U, destination, source2, function2 == 0U ? 0x20U : 0U);
}

[[nodiscard]] std::optional<std::uint32_t> decompress_quadrant1(std::uint16_t bits,
                                                                std::uint8_t function3) noexcept {
    ...
    if (function3 == 3U) {  // C.ADDI16SP / C.LUI
        if (destination == 2U) {
            const auto encoded = (static_cast<std::uint64_t>((bits >> 12U) & 1U) << 9U)
    ...
    if (function3 == 6U || function3 == 7U) {  // C.BEQZ / C.BNEZ
        ...
        return encode_b(function3 == 6U ? 0U : 1U, source, 0U, branch_immediate(bits));
    }
    return std::nullopt;
}

[[nodiscard]] std::optional<std::uint32_t> decompress_cr(std::uint16_t bits) noexcept {
    const auto destination = full_register(bits, 7U);
...
[[nodiscard]] std::optional<std::uint32_t> decompress_quadrant2(std::uint16_t bits,
                                                                std::uint8_t function3) noexcept {
    const auto destination = full_register(bits, 7U);
    const auto source2 = full_register(bits, 2U);
    ...
    if (function3 == 1U || function3 == 3U) {  // C.FLDSP / C.LDSP
        const auto immediate = (static_cast<std::uint64_t>((bits >> 2U) & 0x7U) << 6U)
                               | (static_cast<std::uint64_t>((bits >> 12U) & 1U) << 5U)
                               | (static_cast<std::uint64_t>((bits >> 5U) & 0x3U) << 3U);
        if (function3 == 3U && destination == 0U) {
            return std::nullopt;
        }
        return encode_i(function3 == 1U ? 0x07U : 0x03U, destination, 3U, 2U, immediate);
    }
    if (function3 == 2U) {  // C.LWSP
        if (destination == 0U) {
            return std::nullopt;
        }
        const auto immediate = (static_cast<std::uint64_t>((bits >> 2U) & 0x3U) << 6U)
                               | (static_cast<std::uint64_t>((bits >> 12U) & 1U) << 5U)
    ...
    if (function3 == 5U || function3 == 7U) {  // C.FSDSP / C.SDSP
        const auto immediate = (static_cast<std::uint64_t>((bits >> 7U) & 0x7U) << 6U)
                               | (static_cast<std::uint64_t>((bits >> 10U) & 0x7U) << 3U);
        return encode_s(function3 == 5U ? 0x27U : 0x23U, 3U, 2U, source2, immediate);
    }
    if (function3 == 6U) {  // C.SWSP
        const auto immediate = (static_cast<std::uint64_t>((bits >> 7U) & 0x3U) << 6U)
