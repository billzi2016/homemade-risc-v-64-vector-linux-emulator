...
[[nodiscard]] BusError make_error(BusErrorCode code,
...
AccessResult Bus::register_region(std::shared_ptr<AddressRegion> region) {
    ...
    std::sort(regions_.begin(), regions_.end(), [](const auto& lhs, const auto& rhs) {
...
Bus::LocatedRegion Bus::locate(PhysicalAddress address, AccessWidth width) {
    ...
    if (length == 0U) {
        return LocatedRegion{
            nullptr,
            0U,
            AccessResult::failure(make_error(BusErrorCode::InvalidWidth,
                                             address,
                                             0U,
                                             {},
    ...
    if (address.value() > maximum - (length - 1U)) {
        return LocatedRegion{nullptr,
                             0U,
                             AccessResult::failure(make_error(BusErrorCode::AddressOverflow,
                                                              address,
                                                              length,
                                                              {},
                                                              "物理访问末地址超出 64 位地址空间"))};
    ...
    for (const auto& region : regions_) {
        ...
        if (!region->range().contains(address, length)) {
            return LocatedRegion{nullptr,
                                 0U,
                                 AccessResult::failure(make_error(BusErrorCode::OutOfBounds,
                                                                  address,
                                                                  length,
                                                                  region->name(),
                                                                  "访问跨越物理区域末边界"))};
        ...
        return LocatedRegion{
            region.get(), region->range().offset_of(address), AccessResult::success()};
    }
    ...
    return LocatedRegion{
        ...
        AccessResult::failure(make_error(
            BusErrorCode::Unmapped, address, length, {}, "物理地址没有映射到任何已注册区域"))};
}
...
AccessResult Bus::write(PhysicalAddress address,
                        AccessWidth width,
                        std::uint64_t value,
                        AccessType type) {
    std::lock_guard<std::mutex> transaction_lock{transaction_mutex_};
...
AccessResult Bus::compare_exchange(PhysicalAddress address,
                                   AccessWidth width,
    ...
    const auto result =
        located.region->compare_exchange(located.offset, width, expected, desired, type);
...
LoadReservedResult Bus::load_reserved(PhysicalAddress address, AccessWidth width) {
    ...
    reservation_ =
        ReservationRecord{token, address, reservation_base(address), kReservationGranuleBytes};
    return LoadReservedResult{loaded, token};
}

AccessResult Bus::store_conditional(ReservationToken token,
                                    PhysicalAddress address,
                                    AccessWidth width,
                                    std::uint64_t value) {
    std::lock_guard<std::mutex> transaction_lock{transaction_mutex_};
    ...
    const auto stored = located.region->write(located.offset, width, value, AccessType::Atomic);
    if (!stored.ok()) {
        return stored;
    }
...
void Bus::invalidate_reservation_locked(PhysicalAddress address, std::uint64_t length) noexcept {
