...
class Mmu final {
   public:
    ...
    [[nodiscard]] TranslationResult translate(std::uint64_t virtual_address,
                                              bus::AccessWidth width,
                                              MmuAccessKind kind,
                                              MmuContext context);

    // 执行 SFENCE.VMA 的 TLB 失效。空 VA 表示全地址，空 ASID 表示全地址空间。
    void sfence_vma(std::optional<std::uint64_t> virtual_address,
   ...
   private:
    ...
    [[nodiscard]] std::optional<bus::PhysicalAddress> lookup_tlb(std::uint64_t virtual_address,
                                                                 MmuAccessKind kind,
                                                                 MmuContext context);

    [[nodiscard]] PageWalkResult walk_page_table(std::uint64_t virtual_address,
                                                 MmuAccessKind kind,
                                                 MmuContext context);

    void fill_tlb(std::uint64_t virtual_address,
                  std::uint8_t level,
