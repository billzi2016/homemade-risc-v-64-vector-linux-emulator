...
[[nodiscard]] FloatingResult floating_add(
    FloatingFormat format,
    std::uint64_t lhs,
    std::uint64_t rhs,
    FloatingRoundingMode rounding) noexcept;
[[nodiscard]] FloatingResult floating_subtract(
    FloatingFormat format,
    std::uint64_t lhs,
    std::uint64_t rhs,
    FloatingRoundingMode rounding) noexcept;
[[nodiscard]] FloatingResult floating_multiply(
    FloatingFormat format,
    std::uint64_t lhs,
    std::uint64_t rhs,
    FloatingRoundingMode rounding) noexcept;
[[nodiscard]] FloatingResult floating_divide(
    FloatingFormat format,
    std::uint64_t lhs,
    std::uint64_t rhs,
    FloatingRoundingMode rounding) noexcept;
[[nodiscard]] FloatingResult floating_square_root(
    FloatingFormat format,
    std::uint64_t operand,
    FloatingRoundingMode rounding) noexcept;

// 四种 RISC-V FMA 通过两个显式取反开关共享一次舍入的精确执行路径。
[[nodiscard]] FloatingResult floating_fused_multiply_add(
    FloatingFormat format,
...
[[nodiscard]] std::uint64_t floating_sign_inject(
    FloatingFormat format,
    std::uint64_t magnitude,
    std::uint64_t sign_source,
    std::uint8_t operation) noexcept;
[[nodiscard]] FloatingResult floating_minimum_maximum(
    FloatingFormat format,
    std::uint64_t lhs,
    std::uint64_t rhs,
    bool maximum) noexcept;
[[nodiscard]] FloatingComparisonResult floating_compare(
    FloatingFormat format,
    std::uint64_t lhs,
    std::uint64_t rhs,
...
[[nodiscard]] FloatingResult floating_convert_format(
    ...
    std::uint64_t operand,
    FloatingRoundingMode rounding) noexcept;
[[nodiscard]] FloatingResult floating_from_integer(
    FloatingFormat destination,
    std::uint64_t operand,
