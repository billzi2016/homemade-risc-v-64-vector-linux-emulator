...
class AddressRegion {
   public:
    AddressRegion(std::string name, AddressRange range) : name_(std::move(name)), range_(range) {
    }
    ...
    [[nodiscard]] const AddressRange& range() const noexcept {
        return range_;
    }

    [[nodiscard]] virtual AccessResult read(std::uint64_t offset,
                                            AccessWidth width,
                                            AccessType type) = 0;

    [[nodiscard]] virtual AccessResult write(std::uint64_t offset,
                                             AccessWidth width,
                                             std::uint64_t value,
                                             AccessType type) = 0;

    // 原子比较并更新必须在区域内部成为不可分割事务；value 返回提交时观察到的旧值。
    [[nodiscard]] virtual AccessResult compare_exchange(std::uint64_t offset,
                                                        AccessWidth width,
