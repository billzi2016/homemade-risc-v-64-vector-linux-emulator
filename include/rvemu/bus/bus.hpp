...
class Bus final {
   public:
    ...
    [[nodiscard]] AccessResult read(PhysicalAddress address, AccessWidth width, AccessType type);

    [[nodiscard]] AccessResult write(PhysicalAddress address,
                                     AccessWidth width,
                                     std::uint64_t value,
                                     AccessType type);

    [[nodiscard]] AccessResult compare_exchange(PhysicalAddress address,
                                                AccessWidth width,
    ...
    [[nodiscard]] LoadReservedResult load_reserved(PhysicalAddress address, AccessWidth width);

    // 无论成功、失败还是地址错误，SC 都消费当前 token；exchanged 表示是否真正写入。
    [[nodiscard]] AccessResult store_conditional(ReservationToken token,
