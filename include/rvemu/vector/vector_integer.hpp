...
enum class VectorIntegerOperation : std::uint8_t { Add, Subtract, Multiply, DivideUnsigned, DivideSigned, RemainderUnsigned, RemainderSigned };
...
[[nodiscard]] std::optional<VectorIntegerInstruction> decode_vector_integer_instruction(std::uint32_t instruction) noexcept;
// 以给定 SEW 计算一个截断结果；除零和 signed overflow 与标量 RISC-V M 扩展一致，不产生宿主异常。
[[nodiscard]] std::uint64_t execute_vector_integer_operation(VectorIntegerOperation operation, std::uint64_t lhs, std::uint64_t rhs, std::uint8_t sew_bits) noexcept;
// 将 5 位 vi 立即数按 RVV 规则符号扩展并截断到 SEW。
[[nodiscard]] std::uint64_t sign_extend_vector_immediate(std::uint8_t immediate, std::uint8_t sew_bits) noexcept;
