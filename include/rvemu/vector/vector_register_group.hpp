...
// 边界：本文件不保存 CSR、不决定 tail/mask 策略，也不执行指令；调用者负责把失败转为相应非法指令或 Trap。
...
public:
    ...
    [[nodiscard]] std::optional<VectorElementLocation> locate(std::uint64_t element_index) const noexcept;
    // 以 RVV 小端布局读取一个最多 64 位的元素；无效索引返回空，永不跨越本组有效范围。
    [[nodiscard]] std::optional<std::uint64_t> read_element(
        const VectorState& state,
        std::uint64_t element_index) const;
    // 以 RVV 小端布局写入一个最多 64 位的元素；返回 false 表示索引无效且没有部分写入。
    [[nodiscard]] bool write_element(
        VectorState& state,
        std::uint64_t element_index,
        std::uint64_t value) const;

    // 按 RVV 掩码规则读取 v0 的第 element_index 位；掩码位与 SEW/LMUL 无关，超出 VLEN 返回空。
    [[nodiscard]] static std::optional<bool> mask_bit(
        const VectorState& state,
        std::uint64_t element_index) noexcept;

    [[nodiscard]] const VectorConfiguration& configuration() const noexcept { return configuration_; }
    [[nodiscard]] std::uint8_t base_register() const noexcept { return base_register_; }
