...
- **CPU-REQ-016**：`fflags=0x001`、`frm=0x002`、`fcsr=0x003` 使用同一个 8 位底层状态；`fflags` 是低五位视图，`frm` 是位 7:5 视图。
- **CPU-REQ-017**：`FS=Off` 时访问上述浮点 CSR 或执行任何读写浮点状态的指令必须触发非法指令，不能由内部寄存器写入口偷偷启用 FS。
- **CPU-REQ-018**：写浮点寄存器、浮点 CSR 或累积异常标志时把已启用的 FS 标记为 Dirty；`mstatus/sstatus.SD` 必须由 FS/VS Dirty 状态派生，不能独立保存。
- **CPU-REQ-019**：`fflags` 仅保存 NV/DZ/OF/UF/NX 五位并按 OR 累积指令异常；无异常结果不得清除历史标志，软件显式写 CSR 才能清除。
- `frm` 保存三位编码。保留值可以被 CSR 写入和读回，但后续使用动态舍入的浮点指令必须因无法解析合法模式而触发非法指令。
